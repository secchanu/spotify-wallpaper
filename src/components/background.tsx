...
const Component: FunctionComponent<Props> = (props) => {
	...
	const audioData = useAudioListener();
	const audioArray = audio ? audioData : Array(128).fill(0);
	...
	useEffect(() => {
		...
		const initAngle = Math.PI / 2 + fan / 2;
		const getXY = (value: number, index: number): [number, number] => {
			const angle = index * fan + initAngle;
			const v = rin + (value > threshold ? value : 0) * rin;
			const x = (width - (left + right)) / 2 + left + Math.cos(angle) * v;
			const y = (height - (top + bottom)) / 2 + top + Math.sin(angle) * v;
			return [x, y];
		};
	...
	useEffect(() => {
		...
		const updateKeepImage = () => {
			setKeepImage((k) => {
				const keepers = images.filter((i) => i !== k);
				const keeper = keepers.at(Math.floor(Math.random() * keepers.length));
				return keeper;
			});
		};
		updateKeepImage();
